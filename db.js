...
if (!process.env.PG_CONNECTION_STRING) {
  console.error('Erro: PG_CONNECTION_STRING não está definida no ambiente.');
  process.exit(1);
}
  ...
  try {
    ...
    client = await retry(
      ...
      },
      {
        ...
        minTimeout: 1000,
        maxTimeout: 10000,
        onRetry: (err) => console.log(`Tentativa de conexão falhou: ${err.message}`),
      }
    );
    ...
    for (const table of tables) {
      console.log(`Importando dados da tabela ${table}...`);
      const res = await client.query(`SELECT * FROM ${table}`);
      const rows = res.rows;

      db.serialize(() => {
        db.run(`DELETE FROM ${table}`, err => {
          if (err) console.error(`Erro ao limpar tabela ${table} no SQLite:`, err);
        });
        rows.forEach(row => {
      ...
      console.log(`Dados da tabela ${table} importados com sucesso.`);
    }
    ...
    if (client) {
      client.release();
      console.log('Conexão com PostgreSQL liberada.');
    }
  }
}
